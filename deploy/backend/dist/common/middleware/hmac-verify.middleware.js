...
const require_hmac_decorator_1 = require("../decorators/require-hmac.decorator");
...
let HmacVerifyMiddleware = class HmacVerifyMiddleware {
    ...
    config;
    reflector;
    logger = new common_1.Logger('HmacVerifyMiddleware');
    constructor(redis, config, reflector) {
        this.redis = redis;
        this.config = config;
    ...
    async use(req, res, next) {
        ...
        if (!signature) {
            if (this.isHmacRequired(req)) {
                return this.fail(res, 'SIGNATURE_REQUIRED', '此接口要求 HMAC 验签');
            }
            return next();
        }
        ...
        const secretKey = this.config.get('HMAC_SECRET');
        if (!secretKey) {
            throw new Error('ConfigurationError: HMAC_SECRET is required');
        }
        const userApiKey = req.header('x-api-key') || '';
        if (!userApiKey && req.method !== 'GET') {
            this.logger.warn(`HMAC verification without api-key for write operation: ${req.method} ${req.path}`);
        }
    ...
    isHmacRequired(req) {
        ...
        for (const layer of route.stack) {
            ...
            if (this.reflector.getAllAndOverride(require_hmac_decorator_1.REQUIRE_HMAC_KEY, [
                handler,
            ]) === true) {
                return true;
            }
            ...
            if (controllerClass &&
                this.reflector.getAllAndOverride(require_hmac_decorator_1.REQUIRE_HMAC_KEY, [
                    controllerClass,
                ]) === true) {
                return true;
            }
