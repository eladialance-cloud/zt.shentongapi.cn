...
let ApiKeyPoolService = ApiKeyPoolService_1 = class ApiKeyPoolService {
    ...
    async deductQuota(keyId, amount) {
        const result = await this.keyRepo
            ...
            .update()
            .set({
            usedQuota: () => 'used_quota + :amount',
            remainingQuota: () => 'remaining_quota - :amount',
            dailyUsedQuota: () => 'daily_used_quota + :amount',
            monthlyUsedQuota: () => 'monthly_used_quota + :amount',
            lastUsedAt: new Date(),
            status: () => "CASE WHEN remaining_quota - :amount <= 0 THEN 'exhausted' ELSE status END",
        })
            .where('id = :id AND remaining_quota >= :amount')
            .setParameters({ id: keyId, amount })
            .execute();
        if (result.affected === 0) {
            const exists = await this.keyRepo.findOne({ where: { id: keyId } });
            if (!exists) {
                business_exception_1.BusinessException.throw(error_constant_1.ErrorCode.NOT_FOUND, 'API Key 不存在');
            }
            business_exception_1.BusinessException.throw(error_constant_1.ErrorCode.FORBIDDEN, '余额不足或扣减失败');
        }
    }
