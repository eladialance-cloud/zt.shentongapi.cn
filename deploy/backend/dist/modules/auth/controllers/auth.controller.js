...
let AuthController = class AuthController {
    ...
    login(dto, req, res) {
        return this.authService.login(dto, getClientIp(req), res);
    }
    refresh(req, dto, res) {
        const refreshToken = req.cookies?.refreshToken || dto?.refreshToken;
        if (!refreshToken) {
            business_exception_1.BusinessException.throw(error_constant_1.ErrorCode.UNAUTHORIZED, '缺少 refreshToken');
        }
