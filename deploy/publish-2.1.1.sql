-- 发布 2.1.1：停用旧记录 + 启用新版本
-- 生产库是 shentong（不是 ai_agent）；platform 必须是 win
-- 用法（服务器上）：sudo docker exec -i shentong-mysql mysql --default-character-set=utf8mb4 -uroot shentong < /tmp/publish-2.1.1.sql
-- 可重复执行：先删同版本记录再插入
-- NOTE: latin1 client charset double-encodes Chinese text; force utf8mb4
SET NAMES utf8mb4;
DELETE FROM client_versions WHERE platform='win' AND version='2.1.1';
...
VALUES ('2.1.1','win','/desktop/ShenTongAI-Setup-2.1.1-x64.exe.zip','合并素材库与素材管理为单一入口；修复官署工作手册被改回模板、删官署误删定时任务、定时任务可能重复执行',0,100,NOW(),1);
