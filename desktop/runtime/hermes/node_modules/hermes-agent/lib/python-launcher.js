...
function getPythonCandidates(platform = process.platform) {
  // Check for bundled Python first (HERMES_PYTHON env var set by .cmd launcher)
  const bundled = process.env.HERMES_PYTHON;
  const bundledResults = [];
  if (bundled) {
    bundledResults.push({ command: bundled, args: [] });
  }
  
  if (platform === "win32") {
    return [
      ...bundledResults,
      // Check runtime/python directory relative to the hermes module
      { command: path.join(__dirname, "..", "..", "..", "python", "python.exe"), args: [] },
      { command: "py", args: ["-3"] },
      { command: "python", args: [] },
