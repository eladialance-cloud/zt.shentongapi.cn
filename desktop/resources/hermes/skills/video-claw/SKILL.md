...
> **运行时来源**：本技能只承载驱动 video-claw 的 Agent 指令与 API 参考（`references/`）。实际代码项目（`video-claw/backend` + `video-claw/frontend`）由桌面端「服务管理」作为独立服务提供（后端 `:8000`、前端 `:3000`），无需在本技能目录内查找源码。
...
### 1. 本地部署（桌面端已自动配置，无需手动初始化）
> **桌面端已自动配置**：本技能随 ShenTong AI 桌面端内置分发，`video-claw/backend/config.yaml` 已由桌面端自动生成——API 服务统一指向平台中转 llm-proxy（`https://zt.shentongapi.cn/api/llm-proxy/v1`），Key 已自动注入，平台模型（LLM / 文生图 / 视频）无需用户手动填写。
> 用户如要求“初始化项目”、“配置项目”、“部署项目”，直接告知：桌面端已自动配置，无需手动初始化；如服务未启动，请提示用户打开桌面端「AI 视频」页一键启动。
...
> **桌面端已自动配置**：默认平台模型（qwen3.8-max / qwen-image-3.0 / wan2.7-i2v 等）均走 llmproxy 提供商，Key 已注入 config.yaml，无需用户填写。仅当用户明确选择第三方平台模型（deepseek/dashscope/ark/kling 等）时才需要检查对应平台 Key。
