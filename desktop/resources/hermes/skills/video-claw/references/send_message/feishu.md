...
```python
message(action="send", filePath="$HERMES_HOME/workspace/temp_imgs/xxx.png", message="图片描述", target="user_open_id")
```

## 发送视频

```python
message(action="send", filePath="$HERMES_HOME/workspace/temp_imgs/xxx.mp4", message="视频描述", target="user_open_id")
```
...
2. **下载到本地**：生成媒体后必须先下载到 `$HERMES_HOME/workspace/temp_imgs/` 目录
...
temp_dir = os.path.join(os.getenv("HERMES_HOME", os.path.expanduser("~/.hermes")), "workspace", "temp_imgs")
...
| 找不到文件 | 路径不存在 | 确认 `$HERMES_HOME/workspace/temp_imgs/` 目录下有文件 |
