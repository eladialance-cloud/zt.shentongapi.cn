...
# --- edict runtime path bootstrap (embedded python compat) ---
import sys as _edict_sys, pathlib as _edict_pl
_EDICT_SCRIPTS_DIR = str(_edict_pl.Path(__file__).resolve().parent)
if _EDICT_SCRIPTS_DIR not in _edict_sys.path:
    _edict_sys.path.insert(0, _EDICT_SCRIPTS_DIR)
...
BASE = pathlib.Path(os.environ['EDICT_HOME']) if 'EDICT_HOME' in os.environ else pathlib.Path(__file__).parent.parent
DATA = BASE / 'data'
