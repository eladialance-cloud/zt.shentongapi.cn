...
>
> **绝对禁止的标题：**
> - ❌ `"全面审查/Users/bingsen/clawd/hermes-sansheng-liubu/…"` （含文件路径）
...
> ⚠️ **看板操作由编排器代写（kanban_update.py CLI）**，你不需要自己执行；不要自己读写 JSON 文件！
...
> 🚨 **你在处理每个任务的每个关键步骤时，都要说明当前进展（编排器会代写 progress 命令同步到看板）。**
