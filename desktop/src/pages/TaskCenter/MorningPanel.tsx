/**
 * 天下要闻（edict 原版 MorningPanel 照搬 + 深瞳 IPC 适配）
 * 数据源：edict:morning-brief / edict:morning-brief/config / save / refresh
 *        onEdictMorningUpdated 实时推送；订阅配置本地 state 管理（与 store 脱钩）
 */
...
import {
  ...
  edictMorningBrief,
  edictMorningConfig,
  edictSaveMorningConfig,
  edictRefreshMorning,
  onEdictMorningUpdated,
} from "@/api/edict-api";
import type { EdictMorningBrief, EdictMorningNewsItem, EdictSubConfig } from "@shared/edict-types";
...
export default function MorningPanel() {
  const [morningBrief, setMorningBrief] = useState<EdictMorningBrief | null>(null);
  const [subConfig, setSubConfig] = useState<EdictSubConfig | null>(null);
  const [available] = useState<boolean>(() => isEdictAvailable());

  const [showConfig, setShowConfig] = useState(false);
  const [localConfig, setLocalConfig] = useState<EdictSubConfig | null>(null);
  ...
  useEffect(() => {
    ...
    void loadMorning();
    void loadSubConfig();
    const off = onEdictMorningUpdated((brief) => setMorningBrief(brief));
    return () => off();
  }, [available]);
  ...
  const loadMorning = async () => {
    try {
      const brief = await edictMorningBrief();
      setMorningBrief(brief);
    } catch (err) {
      console.warn("[MorningPanel] 加载要闻失败:", err);
    }
  };

  const loadSubConfig = async () => {
    try {
      const cfg = await edictMorningConfig();
      setSubConfig(cfg);
    } catch (err) {
      console.warn("[MorningPanel] 加载订阅配置失败:", err);
    }
  };
  ...
  const refreshNews = async () => {
    ...
    try {
      ...
      pollRef.current = setInterval(async () => {
        ...
        try {
          const fresh = await edictMorningBrief();
          if (fresh.generated_at && fresh.generated_at !== lastDate) {
            if (pollRef.current) clearInterval(pollRef.current);
            pollRef.current = null;
            setRefreshing(false);
            setRefreshLabel("⟳ 立即采集");
            setMorningBrief(fresh);
            toast("✅ 天下要闻已更新", "ok");
          } else {
  ...
  const saveConfig = async () => {
    if (!localConfig) return;
    try {
      const r = await edictSaveMorningConfig(localConfig);
      if (r.ok) {
        toast("订阅配置已保存", "ok");
        void loadSubConfig();
      } else {
      ...
      {!Object.keys(cats).length ? (
        ...
        <div className="mb-cats">
          {Object.entries(cats).map(([cat, items]) => {
            ...
            const scored = (items as EdictMorningNewsItem[])
              .map((item) => {
