  ...
  useEffect(() => {
    ...
    const loadFile = async (): Promise<void> => {
      ...
      if (isImageFile(filePath)) {
        const imageData = await hermesAPI.readImageFile(filePath);
        if (cancelled) return;
        if (imageData === null) {
      ...
      const result = await hermesAPI.readFile(filePath, 102400);
      if (cancelled) return;
      if (result === null) {
        setError(t("worktree.errorLoading"));
      } else {
        ...
        <div className="file-viewer-header">
          ...
          <div className="file-viewer-actions">
            <button
              ...
              onClick={() => hermesAPI.openFileInEditor(filePath)}
