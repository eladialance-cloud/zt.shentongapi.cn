...
function TreeItem({
  ...
  const loadChildren = useCallback(async () => {
    ...
    setIsLoading(true);
    const result = await hermesAPI.readDirectory(fullPath);
    if (result) {
...
export const WorktreePanel = memo(function WorktreePanel({
  ...
  useEffect(() => {
    ...
    const loadRoot = async (): Promise<void> => {
      const result = await hermesAPI.readDirectory(folderPath);
      if (cancelled) return;
      if (result === null) {
  ...
  const handleOpenTerminal = async (): Promise<void> => {
    setTerminalError(null);
    const opened = await hermesAPI.openTerminal(folderPath);
