...
import { useLightboxClose } from "./useLightboxClose";
import type { MediaToken } from "./mediaUtils";
...
function useMediaContextMenu(
  ...
  return (event) => {
    ...
    hermesAPI.showMediaMenu(token.src, token.name, {
...
export function DownloadChip({
  ...
  return (
    <button
      ...
      onClick={() => hermesAPI.saveMediaFile(token.src, token.name)}
      onContextMenu={onContextMenu}
    >
      <Download size={14} />
      {token.name}
