...
function isToolRow(m: ChatMessage): m is ToolCallMessage | ToolResultMessage {
  const k = (m as { kind?: string } | null | undefined)?.kind;
...
interface MessageListProps {
  ...
  /** Optional renderer-local responder for an inline clarify card. */
...
export const MessageList = memo(function MessageList({
  ...
  const visibleMessages = useMemo(
    () =>
      messages
        // 脏历史可能带 null/undefined 行，先剔掉
        .filter((m) => !!m)
        .filter((m) => {
          if (!isBubble(m)) return true;
          // content 可能来自脏历史（非字符串），先收敛再判断，别在 filter 里抛
          return !!m.error || String(m.content ?? "").trim().length > 0;
        }),
    [messages],
  );

  const lastBubble = [...messages].filter((m) => !!m).reverse().find(isBubble);
