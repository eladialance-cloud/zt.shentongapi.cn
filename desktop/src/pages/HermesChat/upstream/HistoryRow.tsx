...
import { Brain, ChevronRight, Wrench } from "./icons";
import { OrbLoader } from "./OrbLoader";
import { useI18n } from "./useI18n";
import { AttachmentChip } from "./AttachmentChip";
import { ToolGlyph, humanizeToolName } from "./toolMeta";
...
/** args / content 可能来自脏历史数据（对象、null），先收敛成字符串再展示 */
...
function summariseArgs(args: unknown): string {
  ...
  const flat = asText(args).replace(/\s+/g, " ").trim();
...
function isToolCall(msg: ToolItem | null | undefined): msg is ToolCallMessage {
  return !!msg && (msg as { kind?: string }).kind === "tool_call";
}
...
export function orderToolActivityItems(items: ToolItem[]): ToolItem[] {
  // 脏历史里可能混入 null/undefined 行，先剔掉再配对，避免整页渲染被带崩
  const list = items.filter((item): item is ToolItem => !!item);
...
function itemDetail(msg: ToolItem | null | undefined): string {
  if (!msg) return "";
      ...
      >
        <div className="chat-tool-collapse-inner">
          <div className="chat-tool-item-body">
            ...
            >
              {call ? asText(msg.args) || "(no arguments)" : asText(msg.content) || "(empty)"}
            </pre>
          </div>
        </div>
      </div>
