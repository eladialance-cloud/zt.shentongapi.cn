...
import {
  download as downloadRuntime,
  cancelDownload,
} from "./runtime-downloader";
...
function registerIpcHandlers(): void {
  ...
  ipcMain.handle("service:start", (_event, name: ServiceName) =>
    serviceManager.start(name),
  );
  ipcMain.handle("service:stop", (_event, name: ServiceName) =>
    serviceManager.stop(name),
  );
  ipcMain.handle("service:restart", (_event, name: ServiceName) =>
    serviceManager.restart(name),
  );
  ipcMain.handle("service:checkEnv", () => serviceManager.checkEnvironment());
  ipcMain.handle("service:install", (_event, name: ServiceName) =>
  ...
  serviceManager.on(
    "status-changed",
    (name: ServiceName, status: string, info: unknown) => {
  ...
  ipcMain.handle(
    ...
    async () => (await getDeviceFingerprint()).fingerprint,
  );
  ...
  ipcMain.handle(
    "db:initialize",
    async (_event, userToken: string): Promise<boolean> => {
      try {
  ...
  ipcMain.handle(
    "syncQueue:enqueue",
    async (_event, item: SyncQueueItem): Promise<number> => {
  ...
  ipcMain.handle(
    ...
    async (_event, limit: number): Promise<SyncQueueRow[]> => {
  ...
  ipcMain.handle(
    "syncQueue:exists",
    async (_event, client_txn_id: string): Promise<boolean> => {
