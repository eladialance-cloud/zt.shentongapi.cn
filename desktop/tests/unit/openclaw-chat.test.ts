...
describe('createLocalOpenClawWsCaller WS 网关集成', () => {
  ...
  it('WS 网关：携带 modelId 时先 sessions.patch 写入会话模型，再 chat.send（桌面端选择真实生效）', async () => {
    const base = await startWsMock(WS_MOCK_MODEL);
    const caller = createLocalOpenClawWsCaller('http://127.0.0.1:' + base);
    const chunks: string[] = [];
    for await (const c of caller(
      { text: 'hi', token: 'x', sessionId: 9, modelId: 'deepseek-chat' },
      () => {},
      new AbortController().signal,
    )) {
      chunks.push(c);
    }
    assert.ok(chunks.length > 0, '应正常收到回复流');
    assert.match(chunks[0], /model=deepseek-chat/, 'sessions.patch 应携带用户选择的模型');
  });
});
...
const WS_MOCK_MODEL = "const http = require('node:http');\nconst { WebSocketServer } = require('ws');\nconst srv = http.createServer();\nconst wss = new WebSocketServer({ server: srv });\nlet sessionKey = 'agent:main:dashboard:model';\nlet lastModel = '';\nfunction push(ws, frame) { ws.send(JSON.stringify(frame)); }\nwss.on('connection', (ws) => {\n  ws.on('message', (raw) => {\n    const msg = JSON.parse(raw.toString());\n    if (msg.method === 'connect') {\n      ws.send(JSON.stringify({ type: 'res', id: msg.id, ok: true, payload: { type: 'hello-ok', protocol: 4 } }));\n    } else if (msg.method === 'sessions.create') {\n      ws.send(JSON.stringify({ type: 'res', id: msg.id, ok: true, payload: { key: sessionKey } }));\n    } else if (msg.method === 'sessions.patch') {\n      lastModel = String(msg.params.model || '');\n      ws.send(JSON.stringify({ type: 'res', id: msg.id, ok: true, payload: { ok: true } }));\n    } else if (msg.method === 'chat.send') {\n      ws.send(JSON.stringify({ type: 'res', id: msg.id, ok: true, payload: { runId: msg.params.idempotencyKey, status: 'started' } }));\n      const rid = msg.params.idempotencyKey;\n      push(ws, { type: 'event', event: 'agent', payload: { runId: rid, stream: 'assistant', data: { text: 'model=' + lastModel, delta: 'model=' + lastModel } } });\n      push(ws, { type: 'event', event: 'chat', payload: { runId: rid, state: 'final', stopReason: 'stop', message: { role: 'assistant', content: [{ type: 'text', text: 'model=' + lastModel }] } } });\n      push(ws, { type: 'event', event: 'agent', payload: { runId: rid, stream: 'lifecycle', data: { phase: 'end', stopReason: 'stop' } } });\n    }\n  });\n});\nsrv.listen(0, '127.0.0.1', () => console.log('PORT=' + srv.address().port));";
