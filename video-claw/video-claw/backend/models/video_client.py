...
try:
    ...
    from models.video_llmproxy import LlmProxyVideoClient
except ImportError:
    ...
    from video_llmproxy import LlmProxyVideoClient
...
class VideoClient:
    ...
    def __init__(
        ...
        llmproxy_api_key: Optional[str] = None,
        llmproxy_base_url: Optional[str] = None,
    ):
        ...
        self._llmproxy_api_key = llmproxy_api_key or Config.LLMPROXY_API_KEY
        self._llmproxy_base_url = llmproxy_base_url or Config.LLMPROXY_BASE_URL

        self._dashscope_client = None
        self._kling_client = None
        self._seedance_client = None
        self._llmproxy_client = None
    ...
    @property
    def llmproxy_client(self):
        if self._llmproxy_client is None:
            self._llmproxy_client = LlmProxyVideoClient(
                api_key=self._llmproxy_api_key,
                base_url=self._llmproxy_base_url,
            )
        return self._llmproxy_client
    ...
    def generate_video(
        ...
        if Config.is_llmproxy_model(model):
            return self._generate_llmproxy(
                prompt,
                image_path,
                save_path,
                model,
