...
export class RagService {
  ...
  async retrieve(userId: number, query: RetrieveQuery): Promise<RetrieveResult> {
    const { knowledgeBaseId, query: queryText, topK = 5 } = query;

    if (!queryText || !queryText.trim()) {
      throw new HttpException('鏌ヨ鏂囨湰涓嶈兘涓虹┖', HttpStatus.BAD_REQUEST);
    }

    // 1. 楠岃瘉鐭ヨ瘑搴撳綊灞?    const kb = await this.kbRepo.findOne({
      where: { id: knowledgeBaseId, userId },
    });
    if (!kb) {
      throw new NotFoundException(`鐭ヨ瘑搴?${knowledgeBaseId} 涓嶅瓨鍦ㄦ垨鏃犳潈璁块棶`);
    }
    ...
    if (cached) {
      const parsed = JSON.parse(cached) as RetrieveResult;
      return { ...parsed, cached: true };
    }

    // 3. 浠?knowledge_base_chunks 琛ㄦ绱?    const matches = await this.searchChunks(knowledgeBaseId, queryText, topK);

    const result: RetrieveResult = {
      query: queryText,
      matches,
      message: matches.length > 0 ? `鎵惧埌 ${matches.length} 涓浉鍏虫枃妗ｇ墖娈礰 : '鏈壘鍒扮浉鍏虫枃妗ｇ墖娈?,
      cached: false,
    };

    // 4. 鍐欏叆 Redis 缂撳瓨锛圱TL 60 绉掞級
    try {
      await this.redis.set(cacheKey, JSON.stringify(result), 60);
    } catch (err) {
      this.logger.warn(`缂撳瓨鍐欏叆澶辫触: ${(err as Error).message}`);
    }
  ...
  async augmentPrompt(
    userId: number,
    data: { query: string; retrievedDocs: ChunkMatchResult[] },
    ...
    const contextBlocks = retrievedDocs.map((doc, index) => {
      const title = doc.documentName || `鏂囨。${doc.documentId}`;
      return `[鏂囨。${index + 1}] 鏍囬: ${title}\n鍐呭: ${doc.content}`;
    });

    const augmentedPrompt = [
      '鍩轰簬浠ヤ笅鐭ヨ瘑搴撳唴瀹瑰洖绛旂敤鎴烽棶棰橈細',
      '',
      ...contextBlocks,
      '',
      `鐢ㄦ埛闂: ${query}`,
  ...
  async indexDocument(userId: number, data: IndexDocumentData): Promise<IndexResult> {
    const {
      knowledgeBaseId,
      documentId,
      content,
    ...
    if (!content || !content.trim()) {
      throw new HttpException('鏂囨。鍐呭涓嶈兘涓虹┖', HttpStatus.BAD_REQUEST);
    }
    ...
    if (!doc) {
      throw new NotFoundException(`鏂囨。 ${documentId} 涓嶅瓨鍦ㄤ簬鐭ヨ瘑搴?${knowledgeBaseId}`);
    }

    // 3. 鏂囨湰鍒嗙墖
    const chunks = this.splitTextIntoChunks(content, chunkSize, overlap);

    // 4. 浣跨敤浜嬪姟锛氬厛鍒犻櫎鏃у垎鐗囷紝鍐嶅啓鍏ユ柊鍒嗙墖
    const result = await this.dataSource.transaction(async (manager) => {
      // 鍒犻櫎璇ユ枃妗ｇ殑鏃у垎鐗?      await manager.delete(KnowledgeBaseChunkEntity, { documentId });

      // 鏇存柊鏂囨。鐘舵€佷负 processing
      ...
      // 鍒涘缓鏂板垎鐗?      const chunkEntities: KnowledgeBaseChunkEntity[] = chunks.map((chunk, index) => {
        const entity = new KnowledgeBaseChunkEntity();
    ...
    // 5. 鏇存柊鐭ヨ瘑搴撶粺璁?    await this.updateKnowledgeBaseStats(knowledgeBaseId);

    // 6. 娓呴櫎妫€绱㈢紦瀛?    await this.clearRetrieveCache(userId, knowledgeBaseId);

    this.logger.log(
      `鏂囨。 ${documentId} 绱㈠紩瀹屾垚: ${result.chunkCount} 涓垎鐗? ${result.totalTokens} tokens`,
    );
  ...
  async reindexKnowledgeBase(
    ...
    // 2. 鏇存柊鐭ヨ瘑搴撶姸鎬佷负 reindexing
    await this.kbRepo.update({ id: knowledgeBaseId }, { status: 'reindexing' });

    try {
      // 3. 鑾峰彇鎵€鏈夋枃妗?      const documents = await this.docRepo.find({
      ...
      if (documents.length === 0) {
        await this.kbRepo.update(
          { id: knowledgeBaseId },
          { status: 'active', documentCount: 0, totalChunks: 0, totalTokens: 0 },
        );
        return {
          knowledgeBaseId,
          documentCount: 0,
      ...
      for (const doc of documents) {
        // 璇诲彇鏂囨。鍐呭锛堣繖閲屼粠鏂囦欢绯荤粺璇诲彇鎴栦娇鐢ㄥ凡瀛樺偍鍐呭锛?        // 鐢变簬 filePath 瀛樺偍鐨勬槸鏂囦欢璺緞锛屾垜浠皾璇曡鍙?        const content = await this.readDocumentContent(doc);

        if (!content || !content.trim()) {
          this.logger.warn(`鏂囨。 ${doc.id} 鍐呭涓虹┖锛岃烦杩嘸);
          continue;
        }
        ...
        await this.dataSource.transaction(async (manager) => {
          // 鍒犻櫎鏃у垎鐗?          await manager.delete(KnowledgeBaseChunkEntity, { documentId: doc.id });

          // 鍒涘缓鏂板垎鐗?          const chunkEntities: KnowledgeBaseChunkEntity[] = chunks.map(
            (chunk, index) => {
              const entity = new KnowledgeBaseChunkEntity();
      ...
      // 5. 鏇存柊鐭ヨ瘑搴撶粺璁″拰鐘舵€?      await this.kbRepo.update(
        { id: knowledgeBaseId },
        {
          status: 'active',
      ...
      this.logger.log(
        `鐭ヨ瘑搴?${knowledgeBaseId} 閲嶆柊绱㈠紩瀹屾垚: ${documents.length} 鏂囨。, ${totalChunks} 鍒嗙墖`,
      );
      ...
      // 閿欒鏃舵仮澶嶇姸鎬?      await this.kbRepo.update({ id: knowledgeBaseId }, { status: 'error' });
      this.logger.error(
        `鐭ヨ瘑搴?${knowledgeBaseId} 閲嶆柊绱㈠紩澶辫触: ${(err as Error).message}`,
      );
      throw new HttpException(
        `閲嶆柊绱㈠紩澶辫触: ${(err as Error).message}`,
  ...
  /**
   * 鍦ㄧ煡璇嗗簱鐨勫垎鐗囦腑鎼滅储鍖归厤鍐呭
   * 浣跨敤 MySQL FULLTEXT 鎼滅储锛屽洖閫€鍒?LIKE 妯＄硦鍖归厤
   */
  private async searchChunks(
    ...
    // 鎻愬彇鍏抽敭璇?    const keywords = this.extractKeywords(queryText);

    if (keywords.length === 0) {
      return [];
    }

    // 鏂规 1: 灏濊瘯 FULLTEXT 鎼滅储
    try {
      ...
      this.logger.warn(
        `FULLTEXT 鎼滅储澶辫触锛屽洖閫€鍒?LIKE 鍖归厤: ${(err as Error).message}`,
      );
    }

    // 鏂规 2: LIKE 妯＄硦鍖归厤 + 鍏抽敭璇嶈瘎鍒?    return this.likeSearch(knowledgeBaseId, keywords, topK);
  }
  ...
  private async fulltextSearch(
    ...
    // 浣跨敤鍙傛暟鍖栨煡璇㈤伩鍏?SQL 娉ㄥ叆
    // FULLTEXT 鎼滅储闇€瑕佷娇鐢?NATURAL LANGUAGE MODE 鎴?BOOLEAN MODE
    ...
    const rawResults = await this.chunkRepo
      ...
      .addSelect(
        `MATCH(chunk.content) AGAINST (:queryText IN NATURAL LANGUAGE MODE)`,
        'score',
      )
  ...
  /**
   * LIKE 妯＄硦鍖归厤 + 鍏抽敭璇嶈瘎鍒?   * 璇勫垎閫昏緫锛氭瘡涓叧閿瘝鍦ㄧ墖娈典腑鍑虹幇鐨勬鏁颁箣鍜?   */
  private async likeSearch(
    knowledgeBaseId: number,
    keywords: string[],
    topK: number,
  ): Promise<ChunkMatchResult[]> {
    // 鏋勫缓鏌ヨ锛氫换涓€鍏抽敭璇嶅尮閰嶇殑鐗囨
    ...
    // 璁＄畻姣忎釜鐗囨鐨勮瘎鍒?    const scored = rawResults.map((row) => {
      const contentLower = row.content.toLowerCase();
      let score = 0;
      for (const kw of keywords) {
        const kwLower = kw.toLowerCase();
        // 缁熻鍏抽敭璇嶅嚭鐜版鏁?        let idx = contentLower.indexOf(kwLower);
        while (idx !== -1) {
          score += 1;
  ...
  /**
   * 浠庢煡璇㈡枃鏈腑鎻愬彇鍏抽敭璇?   * 绠€鍗曞疄鐜帮細鎸夌┖鏍煎拰鏍囩偣鍒嗚瘝锛岃繃婊ゅ仠鐢ㄨ瘝鍜岃繃鐭殑璇?   */
  private extractKeywords(query: string): string[] {
    // 涓枃鍜岃嫳鏂囨贩鍚堝垎璇?    // 鎸夐潪瀛楁瘝鏁板瓧鍜屼腑鏂囧瓧绗﹀垎鍓?    const tokens = query
      .replace(/[^\u4e00-\u9fa5a-zA-Z0-9\s]/g, ' ')
      .split(/\s+/)
      .filter((t) => t.length >= 2);

    // 鍋滅敤璇嶅垪琛?    const stopWords = new Set([
      '鐨?, '浜?, '鍦?, '鏄?, '鎴?, '鏈?, '鍜?, '灏?, '涓?, '浜?, '閮?, '涓€',
      '涓€涓?, '涓?, '涔?, '寰?, '鍒?, '璇?, '瑕?, '鍘?, '浣?, '浼?, '鐫€', '娌℃湁',
      '鐪?, '濂?, '鑷繁', '杩?, '閭?, '瀹?, '浠?, '濂?, '浠?, '浠€涔?, '鎬庝箞',
  ...
  /**
   * 鏂囨湰鍒嗙墖锛堟寜瀛楃鏁板垏鍓诧紝甯?overlap 閲嶅彔锛?   */
  private splitTextIntoChunks(
    ...
    while (start < text.length) {
      ...
      if (chunkText.length > 0) {
        // 绠€鍗曚及绠?token 鏁帮細涓枃绾?1 瀛?= 1 token锛岃嫳鏂囩害 4 瀛楃 = 1 token
  ...
  /**
   * 绠€鍗曚及绠?token 鏁?   * 涓枃锛氱害 1 瀛?= 1.5 tokens
   * 鑻辨枃锛氱害 4 瀛楃 = 1 token
   */
  ...
  private async updateKnowledgeBaseStats(knowledgeBaseId: number): Promise<void> {
    const stats = await this.chunkRepo
      ...
      .where('chunk.knowledge_base_id = :kbId', { kbId: knowledgeBaseId })
      .getRawOne<{ totalChunks: string; totalTokens: string }>();
  ...
  private async readDocumentContent(
    ...
    try {
      ...
      // 浠呭厑璁歌鍙栨枃浠讹紝涓嶅仛閫掑綊澶勭悊
      const content = await fs.readFile(filePath, 'utf-8');
      return content;
    } catch (err) {
      this.logger.warn(
        `璇诲彇鏂囨。 ${doc.id} 鍐呭澶辫触: ${(err as Error).message}`,
      );
      return '';
    }
  }
  ...
  /**
   * 娓呴櫎鎸囧畾鐭ヨ瘑搴撶殑妫€绱㈢紦瀛?   * 浣跨敤 SCAN 鏌ユ壘鍖归厤鐨?key 骞跺垹闄?   */
  private async clearRetrieveCache(
    userId: number,
    knowledgeBaseId: number,
  ): Promise<void> {
    try {
      ...
      this.logger.warn(
        `娓呴櫎妫€绱㈢紦瀛樺け璐? ${(err as Error).message}`,
      );
    }
  }
}
