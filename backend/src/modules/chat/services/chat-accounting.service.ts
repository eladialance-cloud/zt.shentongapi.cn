...
/**
 * 本地直达对话记账服务（v2：扣费收敛到 llm-proxy）
 *
 * 对话本体由 llm-proxy 按「后台定价 × 实际 token」冻结+结算（供应商 Key 在服务器，用户零配置），
 * 本服务只负责：
 * - tool     : 调用管理后台有定价的工作流时，按 pricePerExecution 额外扣费
 * - proxy-key: 返回/生成用户 llm-proxy 静态 Key（客户端注入用）
 * - preferred-model: 保存用户默认对话模型（llm-proxy 收到客户端内部默认模型名时按此解析）
 */
...
export class ChatAccountingService {
  ...
  /** 返回/生成用户 llm-proxy 静态 Key（客户端登录后注入） */
