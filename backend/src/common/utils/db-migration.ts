...
export async function runStartupMigrations(dataSource: DataSource): Promise<void> {
  ...
  try {
    ...
    // 4b. create_oral_workshop_jobs 表：任务级官方音色列（seed-tts-2.0 音色池）
    try {
      const [owVoiceCol] = await queryRunner.query(
        `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
         WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'create_oral_workshop_jobs' AND COLUMN_NAME = 'voice_speaker_id'`
      );
      if (!owVoiceCol) {
        await queryRunner.query(
          `ALTER TABLE create_oral_workshop_jobs ADD COLUMN voice_speaker_id VARCHAR(128) DEFAULT NULL COMMENT '任务级官方音色 speaker_id'`
        );
        logger.log('Added column: create_oral_workshop_jobs.voice_speaker_id');
      }
    } catch (err) {
      logger.warn('create_oral_workshop_jobs.voice_speaker_id 迁移跳过: ' + (err as Error).message);
    }

    // 5. eco_skill_packages 表（技能商店；实体存在但无建表脚本，缺失时启动自动补建）
    const [skillPackagesTable] = await queryRunner.query(
      `SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'eco_skill_packages'`
    );
    if (!skillPackagesTable) {
      ...
      `);
      logger.log('Created table: eco_skill_packages');
    }

    // 5.5 create_hermes_skills 表（Hermes 技能市场；管理端技能包上架时同步生成记录）
    const [hermesSkillsTable] = await queryRunner.query(
      `SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'create_hermes_skills'`
    );
    if (!hermesSkillsTable) {
      ...
      `);
      logger.log('Created table: create_hermes_skills');
    } else {
      const [hsCol] = await queryRunner.query(
        `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
         WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'create_hermes_skills' AND COLUMN_NAME = 'source_package_id'`
      );
      if (!hsCol) {
        await queryRunner.query(
          `ALTER TABLE create_hermes_skills ADD COLUMN \`source_package_id\` BIGINT UNSIGNED DEFAULT NULL COMMENT '关联管理端技能包 ID'`,
        );
        logger.log('Added column: create_hermes_skills.source_package_id');
      }
    }

    // 6. eco_skill_sources 表
    const [skillSourcesTable] = await queryRunner.query(
      `SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'eco_skill_sources'`
    );
    if (!skillSourcesTable) {
      ...
      `);
      logger.log('Created table: eco_skill_sources');
    } else {
      const [ssCatCol] = await queryRunner.query(
        `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
         WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'eco_skill_sources' AND COLUMN_NAME = 'category'`
      );
      if (!ssCatCol) {
        await queryRunner.query(
          `ALTER TABLE eco_skill_sources ADD COLUMN category VARCHAR(64) DEFAULT NULL COMMENT '平台中文分类'`,
        );
        logger.log('Added column: eco_skill_sources.category');
      }
    }

    // 7. eco_skill_install_logs 表
    const [skillInstallLogsTable] = await queryRunner.query(
      `SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'eco_skill_install_logs'`
    );
    if (!skillInstallLogsTable) {
      ...
      `);
      logger.log('Created table: eco_skill_install_logs');
    }
    ...
    // 10. eco_agent_installs 表（桌面端 Agent 安装/卸载记录）
    const [agentInstallsTable] = await queryRunner.query(
      `SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'eco_agent_installs'`
    );
    if (!agentInstallsTable) {
      ...
      `);
      logger.log('Created table: eco_agent_installs');
    }
    ...
    const [hermesTeamCol] = await queryRunner.query(
      `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'create_hermes_call_logs' AND COLUMN_NAME = 'team_id'`
    );
    if (!hermesTeamCol) {
      await queryRunner.query(
        `ALTER TABLE create_hermes_call_logs ADD COLUMN team_id BIGINT DEFAULT NULL COMMENT '关联 OPC 团队 ID' AFTER user_id`
      );
      logger.log('Added column: create_hermes_call_logs.team_id');
    }
    ...
    for (const [colName, colDef] of teamCols) {
      await ensureColumn('task_teams', colName, colDef);
    }

    await queryRunner.query(`CREATE TABLE IF NOT EXISTS task_team_members (
      id BIGINT NOT NULL AUTO_INCREMENT,
    ...
    for (const [colName, colDef] of teamMemberCols) {
      await ensureColumn('task_team_members', colName, colDef);
    }

    await queryRunner.query(`CREATE TABLE IF NOT EXISTS task_team_tasks (
      id BIGINT NOT NULL AUTO_INCREMENT,
      team_id BIGINT NOT NULL COMMENT '团队 ID',
    ...
    for (const [colName, colDef] of teamTaskCols) {
      await ensureColumn('task_team_tasks', colName, colDef);
    }
    // 团队协作流程节点（Hermes 编排时作为任务主干模板；整表替换保存）
    await queryRunner.query(`CREATE TABLE IF NOT EXISTS task_team_workflow_nodes (
      id BIGINT NOT NULL AUTO_INCREMENT,
      team_id BIGINT NOT NULL COMMENT '团队 ID',
      ...
      assignee_member_ids JSON NULL COMMENT '负责成员 ID 列表（task_team_members.id）',
      created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
      updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
      PRIMARY KEY (id),
      INDEX idx_team_workflow_team (team_id)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='团队协作流程节点'`);
    logger.log('Ensured table: task_team_workflow_nodes');
    ...
    for (const [colName, colDef] of teamWorkflowCols) {
      await ensureColumn('task_team_workflow_nodes', colName, colDef);
    }
    // 旧版 task_teams/task_team_members 表（init.sql 结构）含 owner_id/user_id/role 等 NOT NULL 旧列，
    ...
    await adjustLegacyColumn('task_teams', 'owner_id', "BIGINT UNSIGNED DEFAULT NULL COMMENT '团队所有者 ID（旧字段，新版本使用 creator_id）'");
    await adjustLegacyColumn('task_team_members', 'user_id', "BIGINT UNSIGNED DEFAULT NULL COMMENT '用户 ID（旧字段，新版本使用 agent_id）'");
    await adjustLegacyColumn('task_team_members', 'role', "VARCHAR(32) DEFAULT NULL COMMENT '团队内角色（旧字段，新版本使用 role_title）'");
    // Hermes 实例：确保表存在 + 补齐执行目标字段（团队 / N8N 工作流 / 知识库）
    await queryRunner.query(`CREATE TABLE IF NOT EXISTS create_hermes_instances (
      id BIGINT NOT NULL AUTO_INCREMENT,
      user_id BIGINT NOT NULL,
      name VARCHAR(64) NOT NULL,
    ...
    for (const [colName, colDef] of hermesInstanceCols) {
      await ensureColumn('create_hermes_instances', colName, colDef);
    }
    ...
    // MCP 官方目录表 eco_mcp_catalog（技能市场-MCP 官方来源）
    await queryRunner.query(`CREATE TABLE IF NOT EXISTS eco_mcp_catalog (
      id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    ...
    logger.log('Ensured table: eco_mcp_catalog');

    // eco_mcp_servers 用户已装实例：来源 + 目录条目关联
    await queryRunner.query(`CREATE TABLE IF NOT EXISTS eco_mcp_servers (
      id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
      user_id BIGINT NOT NULL COMMENT '用户 ID',
      ...
      catalog_id BIGINT DEFAULT NULL COMMENT '关联 eco_mcp_catalog.id',
      created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
      updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
      PRIMARY KEY (id),
    ...
    logger.log('Ensured table: eco_mcp_servers');
    await ensureColumn('eco_mcp_servers', 'source', "VARCHAR(16) NOT NULL DEFAULT 'custom' COMMENT '来源: custom/official/chat'");
    await ensureColumn('eco_mcp_servers', 'catalog_id', "BIGINT DEFAULT NULL COMMENT '关联 eco_mcp_catalog.id'");

    // 唯一索引兜底：同一用户对同一官方目录条目只允许一个实例（幂等复用依赖）
    const [ukMcpRow] = await queryRunner.query(
      `SELECT INDEX_NAME FROM INFORMATION_SCHEMA.STATISTICS
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'eco_mcp_servers' AND INDEX_NAME = 'uk_mcp_user_catalog'`
    );
    if (!ukMcpRow) {
      await queryRunner.query(
        'ALTER TABLE eco_mcp_servers ADD UNIQUE KEY `uk_mcp_user_catalog` (user_id, catalog_id)'
      );
      logger.log('Added unique index: eco_mcp_servers.uk_mcp_user_catalog');
    }
    ...
    // ai_models 表补充模型管理列（实体新增，历史库缺列时自动补齐）
    ...
    for (const [colName, colDef] of legacyModelCols) {
      await ensureColumn('ai_models', colName, colDef);
    }
    // ai_models 表：调用模式/场景标签/计费方式等新列（实体新增，历史库缺列时自动补齐）
    await ensureColumn('ai_models', 'call_mode', "VARCHAR(32) NOT NULL DEFAULT 'text_chat' COMMENT '调用模式(14种字典)'");
    // P5 拆表后：计费/能力/场景列归属 ai_model_pricing（由迁移 0008 建表回填），ai_models 不再补齐
    await ensureColumn('ai_models', 'specs', "JSON NULL COMMENT '动态规格字段值'");
    await ensureColumn('ai_models', 'icon_url', "VARCHAR(512) NULL COMMENT '模型图标URL'");
    await ensureColumn('ai_models', 'remark', "VARCHAR(512) NULL COMMENT '管理员备注'");

    // eco_agents 表补充缺失列（实体新增，历史库缺列时自动补齐）
    ...
    for (const [colName, colDef] of agentCols) {
      await ensureColumn('eco_agents', colName, colDef);
    }
    ...
    // 7. ai_model_providers 表
    const [providersTable] = await queryRunner.query(
      `SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'ai_model_providers'`
    );
    if (!providersTable) {
      await queryRunner.query(`
        CREATE TABLE IF NOT EXISTS ai_model_providers (
          ...
          slug VARCHAR(64) NOT NULL COMMENT '唯一标识(=ai_models.provider)',
      ...
      `);
      logger.log('Created table: ai_model_providers');
    }
    ...
    for (const col of modelCols) {
      const [exists] = await queryRunner.query(
        `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
         WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'ai_models' AND COLUMN_NAME = ?`,
        [col.name]
      );
      if (!exists) {
        await queryRunner.query(
          'ALTER TABLE ai_models ADD COLUMN ' + col.name + ' ' + col.def + ' AFTER ' + col.after
        );
        logger.log('Added column: ai_models.' + col.name);
      }
    }
    ...
    const [priceCol] = await queryRunner.query(
      `SELECT COLUMN_COMMENT AS c FROM INFORMATION_SCHEMA.COLUMNS
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'ai_models' AND COLUMN_NAME = 'price_per_1k_input'`
    );
    if (priceCol && !String(priceCol.c).includes('积分/千token')) {
      await queryRunner.query(
        `UPDATE ai_models SET price_per_1k_input = price_per_1k_input * 100,
                price_per_1k_output = price_per_1k_output * 100
         WHERE price_per_1k_input IS NOT NULL OR price_per_1k_output IS NOT NULL`
      );
      await queryRunner.query(
        `ALTER TABLE ai_models MODIFY COLUMN price_per_1k_input DECIMAL(10,4) DEFAULT NULL COMMENT '输入单价(积分/千token)'`
      );
      await queryRunner.query(
        `ALTER TABLE ai_models MODIFY COLUMN price_per_1k_output DECIMAL(10,4) DEFAULT NULL COMMENT '输出单价(积分/千token)'`
      );
      // P5 拆分表同步：0008 迁移先回填 ai_model_pricing，此处换算后才与 ai_models 同单位，需同步保证一致
      const [pricingTbl] = await queryRunner.query(
        `SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES
         WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'ai_model_pricing'`
      );
      if (pricingTbl) {
        await queryRunner.query(
          `UPDATE ai_model_pricing p JOIN ai_models m ON m.id = p.model_id
           SET p.price_per_1k_input = m.price_per_1k_input,
               p.price_per_1k_output = m.price_per_1k_output`
        );
      }
      logger.log('Migrated ai_models price unit to credits/1k (x100)');
    }

    // 10. 老模型按 provider 分组自动建内置供应商并关联
    const [orphanModels] = await queryRunner.query(
      `SELECT COUNT(*) AS c FROM ai_models WHERE provider_id IS NULL AND provider IS NOT NULL`
    );
    if (Number(orphanModels?.c ?? 0) > 0) {
      ...
      for (const g of groups) {
        await queryRunner.query(
          `INSERT INTO ai_model_providers (name, slug, base_url, api_key, status, connection_status, is_builtin, model_count, created_at, updated_at)
        ...
        const [prov] = await queryRunner.query(
          `SELECT id FROM ai_model_providers WHERE slug = ? AND is_builtin = 1 LIMIT 1`,
          [g.provider]
        );
        if (prov) {
          await queryRunner.query(
      ...
      // 凭据已归属供应商，清空模型表冗余凭据
      await queryRunner.query(
        `UPDATE ai_models m JOIN ai_model_providers p ON p.id = m.provider_id
         SET m.api_endpoint = NULL, m.api_key = NULL`
      );
      // 刷新 model_count
      await queryRunner.query(
        `UPDATE ai_model_providers p
         SET model_count = (SELECT COUNT(*) FROM ai_models m WHERE m.provider_id = p.id)
         WHERE p.is_builtin = 1`
      );
      logger.log('Migrated legacy ai_models to ai_model_providers');
    }
    ...
    // ai_model_providers 表：全局中转标志（严格单全局，唯一索引；幂等）
    await ensureColumn('ai_model_providers', 'is_global', "TINYINT(1) NOT NULL DEFAULT 0 COMMENT '是否全局中转(全站至多1条=1)'");
    // ai_model_providers 表：API 风格/限流/余额监控列（实体新增，幂等）
    await ensureColumn('ai_model_providers', 'api_style', "VARCHAR(32) NULL COMMENT 'API风格: openai_compatible/dashscope_native/anthropic/custom'");
    await ensureColumn('ai_model_providers', 'rate_limit_per_minute', "INT NULL COMMENT '每分钟限流'");
    await ensureColumn('ai_model_providers', 'concurrency_limit', "INT NULL COMMENT '并发上限'");
    await ensureColumn('ai_model_providers', 'balance_url', "VARCHAR(512) NULL COMMENT '余额查询接口'");
    await ensureColumn('ai_model_providers', 'balance_headers', "JSON NULL COMMENT '余额接口请求头'");
    await ensureColumn('ai_model_providers', 'balance_extra', "JSON NULL COMMENT '余额接口附加参数'");
    await ensureColumn('ai_model_providers', 'last_balance', "DECIMAL(12,2) NULL COMMENT '最近一次余额'");
    await ensureColumn('ai_model_providers', 'balance_checked_at', "DATETIME NULL COMMENT '最近余额检查时间'");
    await ensureColumn('ai_model_providers', 'balance_alert_threshold', "DECIMAL(12,2) NULL COMMENT '余额告警阈值'");
    const [globalIdx] = await queryRunner.query(
      `SELECT COUNT(*) AS c FROM information_schema.STATISTICS
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'ai_model_providers' AND INDEX_NAME = 'uk_model_providers_global'`
    );
    if (Number(globalIdx?.c ?? 0) === 0) {
      await queryRunner.query(
        `ALTER TABLE ai_model_providers ADD UNIQUE INDEX uk_model_providers_global ((IF(is_global = 1, is_global, NULL)))`
      );
      logger.log('Created index: ai_model_providers.uk_model_providers_global');
    }

    // ai_models 表：排序权重 + 按次计费（幂等）
    ...
    // ai_llm_files 表（专用文本模型两步式：用户上传文件 -> 上游 file_id 映射）
    const [llmFilesTable] = await queryRunner.query(
      `SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'ai_llm_files'`
    );
    if (!llmFilesTable) {
      await queryRunner.query(`
        CREATE TABLE IF NOT EXISTS ai_llm_files (
          id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
          user_id BIGINT UNSIGNED NOT NULL,
          model_id VARCHAR(64) NOT NULL,
      ...
      `);
      logger.log('Created table: ai_llm_files');
    }
    ...
    // ai_models 表：输入类型/高级能力（「类型=输出类型、能力=输入类型」重构，幂等）
    await ensureColumn('ai_models', 'input_types', "JSON DEFAULT NULL COMMENT '输入类型(多选): text/image/video/audio'");
    await ensureColumn('ai_models', 'advanced_capabilities', "JSON DEFAULT NULL COMMENT '高级能力(多选): function_calling/streaming/reasoning/json_mode'");
    // 存量模型按 model_type 回填输入类型（只处理空值行，幂等）
    await queryRunner.query(
      `UPDATE ai_models SET input_types = JSON_ARRAY('text') WHERE model_type IN ('chat','image','video','tts') AND (input_types IS NULL OR JSON_LENGTH(input_types) = 0)`
    );
    await queryRunner.query(
      `UPDATE ai_models SET input_types = JSON_ARRAY('text','image') WHERE model_type IN ('vision','image_edit') AND (input_types IS NULL OR JSON_LENGTH(input_types) = 0)`
    );
    await queryRunner.query(
      `UPDATE ai_models SET input_types = JSON_ARRAY('text') WHERE input_types IS NULL OR JSON_LENGTH(input_types) = 0`
    );
    await queryRunner.query(
      `UPDATE ai_models SET advanced_capabilities = JSON_ARRAY('function_calling') WHERE supports_functions = 1 AND (advanced_capabilities IS NULL OR JSON_LENGTH(advanced_capabilities) = 0)`
    );
    await queryRunner.query(
      `UPDATE ai_models SET advanced_capabilities = JSON_ARRAY() WHERE advanced_capabilities IS NULL`
    );

    // ===== 资产市场重构 Phase 1：六类资产公共字段补齐（幂等） =====
    // eco_agents：允许挂载 MCP ID 数组 + GitHub topics 快照 + 统一定价
    const assetAgentCols: Array<[string, string]> = [
      ['allowed_mcp_ids', "JSON DEFAULT NULL COMMENT '允许挂载的 MCP 目录 ID 数组(eco_mcp_catalog.id)'"],
    ...
    for (const [colName, colDef] of assetMcpCols) {
      await ensureColumn('eco_mcp_catalog', colName, colDef);
    }

    // eco_skill_packages：GitHub 来源字段
    ...
    for (const [colName, colDef] of assetSkillCols) {
      await ensureColumn('eco_skill_packages', colName, colDef);
    }

    // create_hermes_skills（技能包）：挂载技能 ID + GitHub 来源字段
    const assetHermesCols: Array<[string, string]> = [
      ['skill_ids', "JSON DEFAULT NULL COMMENT '挂载的技能 ID 数组(eco_skill_packages.id)'"],
      ['source_type', "VARCHAR(16) NOT NULL DEFAULT 'manual' COMMENT '来源: github=导入 manual=手工'"],
      ['source_repo', "VARCHAR(512) DEFAULT NULL COMMENT '来源仓库 URL'"],
      ['source_path', "VARCHAR(512) DEFAULT NULL COMMENT '仓库内文件路径'"],
      ['github_topics', "JSON DEFAULT NULL COMMENT 'GitHub 导入时仓库 topics 快照'"],
    ...
    for (const [colName, colDef] of assetHermesCols) {
      await ensureColumn('create_hermes_skills', colName, colDef);
    }

    // eco_plugins（用户自定义插件）：分类 + GitHub 来源字段
    ...
    for (const [colName, colDef] of assetPluginCols) {
      await ensureColumn('eco_plugins', colName, colDef);
    }
    ...
    // ai_models 表：按 model_type 回填 call_mode（只回填空值/默认值，保留管理员手工定制，幂等）
    await queryRunner.query(
      `UPDATE ai_models SET call_mode = CASE model_type
    ...
    logger.log('Backfilled ai_models.call_mode from model_type');

    // ai_models 表：按 call_mode 回填 pricing_mode（只填空值，幂等；映射依据 constants/call-modes.ts recommendedBilling）
    await queryRunner.query(
      `UPDATE ai_models SET pricing_mode = CASE call_mode
    ...
    logger.log('Backfilled ai_models.pricing_mode from call_mode');

    // ai_models 表：video_prices -> video_per_second 折算回填（仅补空且为视频模型；取各分辨率最短时长单价折算积分/秒，幂等）
    const videoRows = await queryRunner.query(
      `SELECT id, video_prices FROM ai_models
    ...
    for (const row of videoRows ?? []) {
      ...
      await queryRunner.query(`UPDATE ai_models SET video_per_second = ? WHERE id = ?`, [
        JSON.stringify(perSecond),
        row.id,
      ]);
    }
    logger.log('Backfilled ai_models.video_per_second from video_prices (shortest-duration rate)');

    // ===== 二期 Task 1：create_briefs / media_assets 建表 + 既有表加列（幂等 DDL） =====

    // create_briefs 表：需求单（云端；一期 local_briefs 为离线 MVP）
    const [create_briefsTable] = await queryRunner.query(
      `SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'create_briefs'`
    );
    if (!create_briefsTable) {
      await queryRunner.query(`CREATE TABLE IF NOT EXISTS create_briefs (
        id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
        user_id BIGINT NOT NULL,
      ...
      logger.log('Created table: create_briefs');
    }
    // create_briefs 补充列：拆解失败原因码 + 派发参数（重拆解用），幂等
    await ensureColumn('create_briefs', 'dispatch_error', "VARCHAR(512) DEFAULT NULL COMMENT 'AI拆解失败原因码'");
    await ensureColumn('create_briefs', 'dispatch_params', "JSON DEFAULT NULL COMMENT '派发参数(executeMode/teamId/agentId)'");
    ...
    const [teamTasksBriefIdCol] = await queryRunner.query(
      `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'task_team_tasks' AND COLUMN_NAME = 'brief_id'`
    );
    if (!teamTasksBriefIdCol) {
      await queryRunner.query(`ALTER TABLE task_team_tasks ADD COLUMN brief_id BIGINT NULL`);
      logger.log('Added column: task_team_tasks.brief_id');
    }

    const [teamTasksExecutionRefCol] = await queryRunner.query(
      `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'task_team_tasks' AND COLUMN_NAME = 'execution_ref'`
    );
    if (!teamTasksExecutionRefCol) {
      await queryRunner.query(`ALTER TABLE task_team_tasks ADD COLUMN execution_ref VARCHAR(128) NULL`);
      logger.log('Added column: task_team_tasks.execution_ref');
    }

    // 既有表加列（幂等）：create_publish_plans 关联任务与素材资产
    const [publishPlansTaskIdCol] = await queryRunner.query(
      `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'create_publish_plans' AND COLUMN_NAME = 'task_id'`
    );
    if (!publishPlansTaskIdCol) {
      await queryRunner.query(`ALTER TABLE create_publish_plans ADD COLUMN task_id BIGINT NULL`);
      logger.log('Added column: create_publish_plans.task_id');
    }

    const [publishPlansAssetIdsCol] = await queryRunner.query(
      `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'create_publish_plans' AND COLUMN_NAME = 'asset_ids'`
    );
    if (!publishPlansAssetIdsCol) {
      await queryRunner.query(`ALTER TABLE create_publish_plans ADD COLUMN asset_ids JSON NULL`);
      logger.log('Added column: create_publish_plans.asset_ids');
    }

    // create_hermes_call_logs：instance_id 允许为空（本地编排上报无实例）+ call_type 增加 orchestrate
    const [hclInstanceCol] = await queryRunner.query(
      `SELECT IS_NULLABLE FROM INFORMATION_SCHEMA.COLUMNS
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'create_hermes_call_logs' AND COLUMN_NAME = 'instance_id'`
    );
    if (hclInstanceCol && hclInstanceCol.IS_NULLABLE === 'NO') {
      await queryRunner.query(`ALTER TABLE create_hermes_call_logs MODIFY COLUMN instance_id BIGINT NULL`);
      logger.log('Modified column: create_hermes_call_logs.instance_id nullable');
    }
    const [hclCallTypeCol] = await queryRunner.query(
      `SELECT COLUMN_TYPE FROM INFORMATION_SCHEMA.COLUMNS
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'create_hermes_call_logs' AND COLUMN_NAME = 'call_type'`
    );
    if (
      hclCallTypeCol &&
      ...
      await queryRunner.query(
        `ALTER TABLE create_hermes_call_logs MODIFY COLUMN call_type ENUM('skill_execute','tool_call','agent_invoke','workflow_run','orchestrate') NOT NULL`
      );
      logger.log('Modified column: create_hermes_call_logs.call_type + orchestrate');
    }
    ...
    await ensureColumn('sedimentation_feed', 'task_id', "VARCHAR(64) DEFAULT NULL COMMENT 'Hermes 任务溯源'");

    // 定时任务表（对话创建 -> 桌面端软件开着时调度 -> Hermes 编排执行）
    await queryRunner.query(`CREATE TABLE IF NOT EXISTS task_scheduled_tasks (
      id BIGINT NOT NULL AUTO_INCREMENT,
      user_id BIGINT NOT NULL COMMENT '用户 ID',
    ...
    const [ttTeamIdCol] = await queryRunner.query(
      `SELECT IS_NULLABLE FROM INFORMATION_SCHEMA.COLUMNS
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'task_team_tasks' AND COLUMN_NAME = 'team_id'`
    );
    if (ttTeamIdCol && String(ttTeamIdCol.IS_NULLABLE).toUpperCase() === 'NO') {
      await queryRunner.query(`ALTER TABLE task_team_tasks MODIFY COLUMN team_id BIGINT NULL`);
      logger.log('Modified column: task_team_tasks.team_id nullable');
    }
    const [ttModeCol] = await queryRunner.query(
      `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'task_team_tasks' AND COLUMN_NAME = 'execute_mode'`
    );
    if (!ttModeCol) {
      await queryRunner.query(
        `ALTER TABLE task_team_tasks ADD COLUMN execute_mode ENUM('team','auto','agent') NOT NULL DEFAULT 'team' COMMENT '执行方式'`
      );
      logger.log('Added column: task_team_tasks.execute_mode');
    }
    const [ttAgentCol] = await queryRunner.query(
      `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'task_team_tasks' AND COLUMN_NAME = 'agent_id'`
    );
    if (!ttAgentCol) {
      await queryRunner.query(
        `ALTER TABLE task_team_tasks ADD COLUMN agent_id BIGINT NULL COMMENT '指定Agent执行'`
      );
      logger.log('Added column: task_team_tasks.agent_id');
    }
    // 口播工坊：create_oral_workshop_jobs.bilingual（双语字幕开关）
    const [owBilingualCol] = await queryRunner.query(
      `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'create_oral_workshop_jobs' AND COLUMN_NAME = 'bilingual'`
    );
    if (!owBilingualCol) {
      await queryRunner.query(
        `ALTER TABLE create_oral_workshop_jobs ADD COLUMN bilingual TINYINT(1) NOT NULL DEFAULT 0 COMMENT '双语字幕开关'`
      );
      logger.log('Added column: create_oral_workshop_jobs.bilingual');
    }

    // 口播工坊：create_oral_workshop_jobs.target_lang（字幕目标语言）
    const [owTargetLangCol] = await queryRunner.query(
      `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'create_oral_workshop_jobs' AND COLUMN_NAME = 'target_lang'`
    );
    if (!owTargetLangCol) {
      await queryRunner.query(
        `ALTER TABLE create_oral_workshop_jobs ADD COLUMN target_lang VARCHAR(16) NULL COMMENT '字幕目标语言(空=中文;en/zh-HK=双语)'`
      );
      logger.log('Added column: create_oral_workshop_jobs.target_lang');
    }

    // 口播工坊：create_oral_workshop_jobs 执行模式（execution_mode / waiting_step）
    const [owExecModeCol] = await queryRunner.query(
      `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'create_oral_workshop_jobs' AND COLUMN_NAME = 'execution_mode'`
    );
    if (!owExecModeCol) {
      await queryRunner.query(
        `ALTER TABLE create_oral_workshop_jobs ADD COLUMN execution_mode VARCHAR(16) NOT NULL DEFAULT 'auto' COMMENT '执行模式:auto自动/manual手动/single单步'`
      );
      logger.log('Added column: create_oral_workshop_jobs.execution_mode');
    }
    const [owWaitingStepCol] = await queryRunner.query(
      `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'create_oral_workshop_jobs' AND COLUMN_NAME = 'waiting_step'`
    );
    if (!owWaitingStepCol) {
      await queryRunner.query(
        `ALTER TABLE create_oral_workshop_jobs ADD COLUMN waiting_step VARCHAR(32) NULL COMMENT '手动模式等待用户放行的步骤'`
      );
      logger.log('Added column: create_oral_workshop_jobs.waiting_step');
    }

    // 口播工坊：create_oral_workshop_jobs 封面设计（主/副标题 + 设计配置）
    const owCoverCols = await queryRunner.query(
      `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'create_oral_workshop_jobs' AND COLUMN_NAME IN ('cover_h1','cover_h2','cover_config')`
    );
    const haveCoverCol = (name: string) => owCoverCols.some((c: { COLUMN_NAME: string }) => c.COLUMN_NAME === name);
    if (!haveCoverCol('cover_h1')) {
      await queryRunner.query(`ALTER TABLE create_oral_workshop_jobs ADD COLUMN cover_h1 VARCHAR(64) NULL COMMENT '封面主标题'`);
      logger.log('Added column: create_oral_workshop_jobs.cover_h1');
    }
    if (!haveCoverCol('cover_h2')) {
      await queryRunner.query(`ALTER TABLE create_oral_workshop_jobs ADD COLUMN cover_h2 VARCHAR(64) NULL COMMENT '封面副标题'`);
      logger.log('Added column: create_oral_workshop_jobs.cover_h2');
    }
    if (!haveCoverCol('cover_config')) {
      await queryRunner.query(`ALTER TABLE create_oral_workshop_jobs ADD COLUMN cover_config TEXT NULL COMMENT '封面设计配置JSON'`);
      logger.log('Added column: create_oral_workshop_jobs.cover_config');
    }
