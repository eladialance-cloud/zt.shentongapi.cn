...
-- =====================================================
-- 1. create_hermes_skills 表：补全缺失字段
...
-- 可通过以下查询检查列是否存在：
--   SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = 'ai_agent' AND TABLE_NAME = 'create_hermes_skills' AND COLUMN_NAME = 'exec_config';
ALTER TABLE `create_hermes_skills` ADD COLUMN `exec_config` JSON NULL COMMENT '技能执行配置（JSON）';
-- 1.2 添加 category 字段（VARCHAR(64)，技能分类）
ALTER TABLE `create_hermes_skills` ADD COLUMN `category` VARCHAR(64) NULL COMMENT '技能分类';
-- 1.3 添加 avg_rating 字段（DECIMAL(3,2)，平均评分）
ALTER TABLE `create_hermes_skills` ADD COLUMN `avg_rating` DECIMAL(3,2) DEFAULT 0.00 COMMENT '平均评分（0-5）';
-- 1.4 添加 rating_count 字段（INT，评分数）
ALTER TABLE `create_hermes_skills` ADD COLUMN `rating_count` INT DEFAULT 0 COMMENT '评分数';
-- 1.5 添加 tags 字段（JSON，标签数组）
ALTER TABLE `create_hermes_skills` ADD COLUMN `tags` JSON NULL COMMENT '标签（JSON 数组）';
-- 1.6 添加 changelog 字段（TEXT，更新日志）
ALTER TABLE `create_hermes_skills` ADD COLUMN `changelog` TEXT NULL COMMENT '更新日志';

-- 为 create_hermes_skills 添加分类索引
ALTER TABLE `create_hermes_skills` ADD INDEX `idx_hermes_skills_category` (`category`);

-- =====================================================
-- 2. create_hermes_skill_ratings 表：完全缺失，需新建
...
-- =====================================================
-- 3. eco_n8n_webhook_logs 表：补全缺失字段
--    Entity: n8n-webhook-log.entity.ts 定义了 signature_provided 和 status 字段
--    现有 SQL 缺失这两个字段
-- =====================================================

-- 3.1 添加 signature_provided 字段（BOOLEAN，是否提供了签名）
ALTER TABLE `eco_n8n_webhook_logs` ADD COLUMN `signature_provided` BOOLEAN NOT NULL DEFAULT FALSE COMMENT '是否提供了签名';
...
ALTER TABLE `create_hermes_call_logs` DROP INDEX `idx_hermes_call_logs_instance`, ADD INDEX `idx_hermes_call_logs_instance_id` (`instance_id`);
-- 5.2 重命名 user_id 索引
ALTER TABLE `create_hermes_call_logs` DROP INDEX `idx_hermes_call_logs_user`, ADD INDEX `idx_hermes_call_logs_user_id` (`user_id`);
-- 5.3 重命名 created_at 索引
ALTER TABLE `create_hermes_call_logs` DROP INDEX `idx_hermes_call_logs_created`, ADD INDEX `idx_hermes_call_logs_created_at` (`created_at`);

-- =====================================================
-- 6. create_hermes_instances 表：确保引擎和字符集一致
--    现有 999_create_missing_tables.sql 已有 ENGINE=InnoDB 但缺少 COLLATE
-- =====================================================
ALTER TABLE `create_hermes_instances` ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
ALTER TABLE `create_hermes_call_logs` ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
ALTER TABLE `create_hermes_skills` ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
-- 执行后可通过以下查询验证：
--
-- 检查 create_hermes_skills 新字段：
--   DESCRIBE create_hermes_skills;
--
-- 检查 create_hermes_skill_ratings 表：
--   DESCRIBE create_hermes_skill_ratings;
--
-- 检查 eco_n8n_webhook_logs 新字段：
--   DESCRIBE eco_n8n_webhook_logs;
--
-- 检查索引名称：
