...
-- 1. 给 create_hermes_skills 表添加新字段（category, avg_rating, rating_count, tags, changelog）
-- 2. 创建 create_hermes_skill_ratings 评分表
-- 3. 预置技能包数据（10条，含分类和标签）

-- ============================================================
-- 1. 扩展 create_hermes_skills 表
-- ============================================================
...
ALTER TABLE `create_hermes_skills` ADD COLUMN `avg_rating` DECIMAL(3,2) DEFAULT 0.00 COMMENT '平均评分';
ALTER TABLE `create_hermes_skills` ADD COLUMN `rating_count` INT DEFAULT 0 COMMENT '评分数';
ALTER TABLE `create_hermes_skills` ADD COLUMN `tags` JSON NULL COMMENT '标签数组';
...
-- 文档处理类
INSERT INTO `create_hermes_skills` (`name`, `description`, `author`, `price_per_minute`, `install_count`, `version`, `is_active`, `category`, `tags`, `avg_rating`, `rating_count`, `exec_config`, `created_at`, `updated_at`) VALUES
...
-- 4. 示例评分数据
-- ============================================================
INSERT INTO `create_hermes_skill_ratings` (`user_id`, `skill_id`, `rating`, `comment`, `created_at`) VALUES
