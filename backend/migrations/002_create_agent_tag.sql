...
-- Agent标签库
CREATE TABLE IF NOT EXISTS `eco_agent_tag` (
...
-- Agent-标签关联表
CREATE TABLE IF NOT EXISTS `eco_agent_tag_map` (
  `id` BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
  `agent_id` BIGINT NOT NULL COMMENT '关联 eco_agents.id',
  `tag_id` BIGINT NOT NULL COMMENT '关联 eco_agent_tag.id',
  `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
