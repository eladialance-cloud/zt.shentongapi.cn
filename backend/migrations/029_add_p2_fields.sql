...
ALTER TABLE oral_workshop_jobs ADD COLUMN dh_generation_mode VARCHAR(8) NOT NULL DEFAULT 'auto' COMMENT '数字人生成方式 auto/cloud/local' AFTER dh_model_version;
-- D3 多镜头拼接（JSON 数组 [{digitalHumanId, seconds}]）
ALTER TABLE oral_workshop_jobs ADD COLUMN shots TEXT DEFAULT NULL COMMENT '多镜头 JSON [{digitalHumanId,seconds}]' AFTER dh_generation_mode;
-- E7 多轨道独立控制：字幕轨开关（默认开）
ALTER TABLE oral_workshop_jobs ADD COLUMN subtitles_enabled TINYINT(1) NOT NULL DEFAULT 1 COMMENT '字幕轨开关(E7)' AFTER bilingual;
-- E7 背景音乐轨开关（默认开；关闭后即使配置了 BGM 也不混入）
ALTER TABLE oral_workshop_jobs ADD COLUMN bgm_enabled TINYINT(1) NOT NULL DEFAULT 1 COMMENT 'BGM 轨开关(E7)' AFTER subtitles_enabled;
-- E4 字幕文本覆盖（多行，每行一条字幕；留空=按文案自动分段）
ALTER TABLE oral_workshop_jobs ADD COLUMN subtitles_override TEXT DEFAULT NULL COMMENT 'E4 字幕文本覆盖' AFTER bgm_enabled;
...
-- F4a 发布账号：publish_plans 关联账号
ALTER TABLE publish_plans ADD COLUMN account_id BIGINT DEFAULT NULL COMMENT '发布账号(F4a)' AFTER publish_status;
