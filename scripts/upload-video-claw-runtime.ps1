...
$serverCmd = 'sudo mkdir -p /opt/shentong/runtime/video-claw/0.1.2 && sudo mv /tmp/video-claw-win-x64.tar.gz /opt/shentong/runtime/video-claw/0.1.2/ && sudo chown -R www-data:www-data /opt/shentong/runtime/video-claw && sudo mkdir -p /opt/shentong/cdn/video-claw/0.1.2 && sudo cp /opt/shentong/runtime/video-claw/0.1.2/video-claw-win-x64.tar.gz /opt/shentong/cdn/video-claw/0.1.2/ && sudo mkdir -p /opt/shentong/updates/runtime/video-claw/0.1.2 && sudo cp /opt/shentong/runtime/video-claw/0.1.2/video-claw-win-x64.tar.gz /opt/shentong/updates/runtime/video-claw/0.1.2/ && sudo chown -R www-data:www-data /opt/shentong/cdn/video-claw /opt/shentong/updates/runtime/video-claw && ls -la /opt/shentong/runtime/video-claw/0.1.2 && cd /tmp && rm -rf vc-chk && mkdir vc-chk && tar -xzf /opt/shentong/runtime/video-claw/0.1.2/video-claw-win-x64.tar.gz -C vc-chk && test -f vc-chk/video-claw.cmd && test -f vc-chk/video-claw-server.js && test -f vc-chk/node/node.exe && test -f vc-chk/python/python.exe && test -f vc-chk/video-claw/video-claw/backend/api_server.py && test -f vc-chk/video-claw/video-claw/frontend/.next/BUILD_ID && echo STRUCTURE_OK && rm -rf vc-chk'
...
$curlCmd = 'curl -sI https://zt.shentongapi.cn/runtime/video-claw/0.1.2/video-claw-win-x64.tar.gz | grep -iE "HTTP/|content-length"'
